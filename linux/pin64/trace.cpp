...
int main(int argc, char *argv[]){
	...
	maxhandle = getdtablesize();
	maxhandle--;
	secfd = shm_open("/pin_shared_mem", O_RDWR, S_IREAD | S_IWRITE);
	if (secfd != -1){
	        dup2(secfd, maxhandle);
		close(secfd);
		secfd = maxhandle;
		pcounter = (unsigned char *)mmap(NULL, SECTION_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, secfd, 0);
		if (pcounter == NULL){
